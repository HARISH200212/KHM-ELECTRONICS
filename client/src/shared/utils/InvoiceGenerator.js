...
export const generateInvoicePDF = (order, invoiceNumber) => {
    ...
    const resolvedInvoiceNo = invoiceNumber || order.invoiceNumber || `INV-${orderId}`;
    ...
    doc.setFontSize(22);
    doc.text(`#${resolvedInvoiceNo.slice(-8).toUpperCase()}`, 20, 39);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(16);
...
export const downloadInvoice = (order, invoiceNumber) => {
    const doc = generateInvoicePDF(order, invoiceNumber);
    doc.save(`Invoice_${invoiceNumber}.pdf`);
};

export const viewInvoice = (order, invoiceNumber) => {
    const doc = generateInvoicePDF(order, invoiceNumber);
    // Open in new window
